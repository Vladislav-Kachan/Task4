...
const Users = observer(() => {
  ...
  const blockUsers = async() => {    
    try { 
      const i = selectedUsers.indexOf(user.user.email);    
      const emails = Array.from(localStorage.getItem('selectedUsers'));      
    ...
    return (
        <div>
          <div className="justify-content-center">
            <Button className="m-2" variant="outline-success" onClick={unlockUsers}>Unlock</Button>
            <Button className="m-2" variant="outline-warning" onClick={blockUsers}>Block</Button>
            <Button className="m-2" variant="outline-danger" onClick={dropUser}>Delete</Button>
          </div>

          <BootstrapTable data={allUsers.user} options={{sortName: 'id', sortOrder: 'asc'}} selectRow={ selectRow } >
